...
from get_predictions import _get_AWS_claude_prediction
...
def summarise_claud(input, index, res):
    ...
    while True:
        try:
            # print("loc: ", loc)
            messages = []
            #[{"content": ("Dokumentera alla händelser du identifierar i texten och inkludera tidpunkten när de sker. Alla relevanta händelser ska dokumenteras. Alla svar skall vara på svenska. Här är materialet du ska behandla  :" + input), "role": "user"}],
            messages.append({
                "role": "user",
                "content": ("Dokumentera alla händelser du identifierar i texten och inkludera tidpunkten när de sker. Alla relevanta händelser ska dokumenteras. Alla svar skall vara på svenska. Här är materialet du ska behandla  :" + input)
            })
            #Turn messages into a JSON object
            ret = _get_AWS_claude_prediction(messages,
                                             context, 
                                             max_tokens=3000, 
