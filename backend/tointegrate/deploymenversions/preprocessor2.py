...
from timelinemaker2 import analyzefromstr
...
def swifthandle(pdf_file, chunk, resind, client, name, resstrings, chunksize, images, project_id):
    raw_document = documentai.RawDocument(content=chunk, mime_type="application/pdf")
    global locind, location_options
        ...
        if(piccand(temp)):
            ...
            while True:
                try:
                    ...
                    locind += 1
                    if(locind == len(location_options)):
                        locind = 0
                    vertexai.init(project=project_id, location=location_options[locind])
...
def ocr_pdf(pdf_file, project_id, location, processor_id, images):
    ...
    for i in range(num_chunks):
        resstrings.append("")
        t = threading.Thread(target=swifthandle, args=(pdf_file, rawdata[i], i,  client, name, resstrings, chunk_size, images, project_id))
        threads.append(t)
        t.start()
