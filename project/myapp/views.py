...
def upload_pdf_file(request):
    if request.method == 'POST' and request.FILES.get('file'):
        ...
        # Create a new instance of the PDFDocument model and save the file
        pdf_document = PDFDocument(file=pdf_file)
        pdf_document.save()
        
        # Convert the ObjectId to a string before returning it in the JSON response
        file_id = str(pdf_document.id)
        
        # Include additional metadata in the JSON response
        response_data = {
            'message': 'PDF file uploaded successfully',
            'file_id': file_id,
            'file_name': pdf_document.file_name,
            'uploaded_at': pdf_document.uploaded_at.strftime('%Y-%m-%d %H:%M:%S')
        }
